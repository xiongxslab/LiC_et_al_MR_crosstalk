...
coloc_abf <- function(i){
  ...
  coloc_dat <- inner_join(Phe1_coloc_dat, Phe2_coloc_dat, by = "SNP", suffix = c("_m6A", paste0("_", Phe2)))
  coloc_res <- coloc.abf(dataset1 = list(snp = coloc_dat$SNP, beta = coloc_dat$beta_m6A, varbeta = coloc_dat$varbeta_m6A,
					 MAF = coloc_dat$MAF_m6A, N = n[[Phe1]][tis], type = "quant"),
			 dataset2 = list(snp = coloc_dat$SNP, beta = coloc_dat[[paste0("beta_", Phe2)]], varbeta = coloc_dat[[paste0("varbeta_", Phe2)]],
					 MAF = coloc_dat[[paste0("MAF_", Phe2)]], N = n[[Phe2]][tis], type = "quant"))
  coloc_results_file <- paste0("~/Coloc/m6A2", Phe2, "/", tis, "_m6A2", Phe2, "_results/", coloc_pairs[i,exposure], "-", coloc_pairs[i,outcome], ".coloc.results")
...
##Colocalization analyses for the same m6A to epigenome MR pairs as further validation
args <- commandArgs(T)
tis <- args[1]
n_chr <- args[2]
...
chr_file <- paste0("~/2SMR/m6A2", Phe2, "/", tis, "_m6A2", Phe2, "_mr/chr", n_chr, ".mr.res")
Phe1_file <- paste0("~/Database/", tis, "_m6A-QTL_all/", tis, ".m6A-QTL.all_chr", n_chr, ".txt")
Phe2_file <- paste0("~/Database/", tis, "_", Phe2, "QTL_all/", tis, ".", Phe2, "QTL.all_chr", n_chr, ".txt")
maf_file <- paste0("~/Database/GTEx_v8/maf/chr", n_chr, ".frq")
out_file <- paste0("~/Coloc/m6A2", Phe2, "/", tis, "_m6A2", Phe2, "_coloc/chr", n_chr, ".coloc.res")
...
if(Phe2 == "ha"){
	...
	Phe2_dat <- fread(Phe2_file, sep = "\t", header = FALSE, col.names = c("Phenotype", "SNP", "beta", "se", "effect_allele", "other_allele", "MAF", "pvalues")) %>%
