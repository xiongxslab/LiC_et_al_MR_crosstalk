...
if(Phe1 == "m6A"){
	Phe_file <- paste0("~/2SMR/m6A2", Phe2, "/", tis, "_m6A-QTL_signif/", tis, ".m6A-QTL.signif_chr", n_chr, ".txt")
}else{
	Phe_file <- paste0("~/2SMR/", Phe1, "2m6A/", tis, "_", Phe1, "QTL_signif/", tis, ".", Phe1, "QTL.signif_chr", n_chr, ".txt")
}
maf_file <- paste0("~/Database/GTEx_v8/maf/chr", n_chr, ".frq")

smr_pairs <- fread(chr_file, sep = "\t", header = FALSE, col.names = c(Phe1, Phe2)) %>% distinct(Phe1)
...
for(i in 1:nrow(smr_pairs)){
  Phe_esd <- Phe_dat %>% filter(Phenotype == smr_pairs[i,Phe1]) %>% select(-Phenotype)
  if(Phe1 == "m6A"){
	  Phe_esd_file <- paste0("~/SMR/m6A2", Phe2, "/", tis, "_m6A-QTL_exp_esd/", smr_pairs[i,"m6A"], ".esd")
  }else{
	  Phe_esd_file <- paste0("~/SMR/", Phe1, "2m6A/", tis, "_", Phe1, "QTL_exp_esd/", smr_pairs[i,Phe1], ".esd")
  }
