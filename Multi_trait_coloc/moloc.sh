...
partition_chrs(){
	...
	cat ~/Database/GWAS/GWAS_metadata.txt |cut -f 1 |while read line; do
...
perform_m6A2ha_moloc(){
	cat ~/Database/GWAS/GWAS_metadata.txt |cut -f 1 |while read line; do
		for tis in Brain Lung Muscle Heart; do
			for n in `seq 22`; do Rscript m6A2epigenome_moloc.R ${tis} ${n} ha ${line}; done
        	done
	done
}
...
perform_m6A2me_moloc(){
        cat ~/Database/GWAS/GWAS_metadata.txt |cut -f 1 |while read line; do
			...
			for n in `seq 22`; do Rscript m6A2epigenome_moloc.R ${tis} ${n} me ${line}; done
                done
        done    
}
...
perform_ha2m6A_moloc(){
        cat ~/Database/GWAS/GWAS_metadata.txt |cut -f 1 |while read line; do
		for tis in Brain Lung Muscle Heart; do
			for n in `seq 22`; do Rscript epigenome2m6A_moloc.R ${tis} ${n} ha ${line}; done
                done
        done
}
...
perform_me2m6A_moloc(){
        cat ~/Database/GWAS/GWAS_metadata.txt |cut -f 1 |while read line; do
                for tis in Lung Muscle; do
			for n in `seq 22`; do Rscript epigenome2m6A_moloc.R ${tis} ${n} me ${line}; done
                done
        done
}
