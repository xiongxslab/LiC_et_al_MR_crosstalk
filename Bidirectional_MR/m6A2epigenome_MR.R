...
perform_mr <- function(i){
	...
	if(length(mr_dat$SNP) >= 1){
		...
		mr_dat_file <- paste0("~/2SMR/m6A2", Phe2, "/", tis, "_m6A2", Phe2, "_dat/", mr_pairs[i,Phe1], "-", mr_pairs[i,Phe2], ".mr.dat")
...
##Perform m6A to epigenome MR across four tissues
args <- commandArgs(T)
tis <- args[1]
n_chr <- args[2]
...
chr_file <- paste0("~/2SMR/m6A2", Phe2, "/", tis, "_m6A2", Phe2, "_pairs/", tis, ".m6A2", Phe2, "_chr", n_chr, ".txt")
Phe1_file <- paste0("~/Database/", tis, "_m6A-QTL_signif/", tis, ".m6A-QTL.signif_chr", n_chr, ".txt")
...
out_file <- paste0("~/2SMR/m6A2", Phe2, "/", tis, "_m6A2", Phe2, "_mr/chr", n_chr, ".mr.res")
...
if(Phe2 == "ha"){
	...
	Phe2_dat <- fread(Phe2_file, sep = "\t", header = FALSE, col.names = c("Phenotype", "SNP", "beta", "se", "effect_allele", "other_allele", "eaf", "pval")) %>%
		mutate(samplesize = n[[Phe2]][tis])
}
print(paste0("chr", n_chr, " input files preparation done"))
