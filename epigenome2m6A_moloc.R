...
##perform multi-trait colocalization analyses between significant epigenome to m6A MR pairs and GWAS
args <- commandArgs(T)
tis <- args[1]
nchr <- args[2]
...
chr_file <- paste0("~/Moloc/", Phe1, "2m6A/", tis, "_", Phe1, "2m6A_pairs/", tis, ".", Phe1, "2m6A_chr", nchr, ".txt")
Phe1_file <- paste0("~/Database/", tis, "_", Phe1, "QTL_all/", tis, ".", Phe1, "QTL.all_chr", nchr, ".txt")
Phe2_file <- paste0("~/Database/", tis, "_m6A-QTL_all/", tis, ".m6A-QTL.all_chr", nchr, ".txt")
...
out_file <- paste0("~/Moloc/", tis, "/", gwasid, "/chr", nchr, ".moloc.pp")
...
if(Phe1 == "ha"){
	...
	Phe1_dat <- fread(Phe1_file, sep = "\t", header = FALSE, col.names = c("Phenotype", "SNP", "BETA", "SE", "effect_allele", "other_allele", "eaf", "pval")) %>%
		inner_join(y = maf, by = "SNP") %>% mutate(BETA = ifelse(effect_allele == A1, BETA, -BETA), N = n[[Phe1]][tis])
}
